        ...
        public static TLockerMapContent Create(string Path)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int x = 0; x < 256; x++)
                    {
                        r.LockerMaps.Add(x, new TLockerMap() {BoxAddr=x, Enable = false, IsUserFixed = false, IsSensorPresent=false, AlamrNro=0, LockerType= TLockerMap.EnumLockerType.NORMAL, TempMin=0, TempMax=0  });
                    }
                    r.Save(Path);
                    return r;
                }
